...
object HANAType {
  ...
  case object REAL extends HANATypeId.SelfIdentified("REAL", 9)
  case object DOUBLE extends HANATypeId.SelfIdentified("DOUBLE", 10)

  final case class DECIMAL(precision: Int, scale: Int) extends HANAType(s"DECIMAL($precision, $scale)")
  case object DECIMAL extends HANATypeId.HigherKinded(11) {
  ...
  case object FLOAT extends HANATypeId.HigherKinded(12) {
    val constructor = Constructor.Unary(LengthFloatParam(53), FLOAT(_))
  }

  case object BOOLEAN extends HANATypeId.SelfIdentified("BOOLEAN", 13)

  final case class VARCHAR(length: Int) extends HANAType(s"VARCHAR($length)")
  case object VARCHAR extends HANATypeId.HigherKinded(14) {
  ...
  case object NVARCHAR extends HANATypeId.HigherKinded(15) {
  ...
  case object VARBINARY extends HANATypeId.HigherKinded(16) {
